...
export default function UnifiedUserProfilePage() {
  ...
  const { userId: currentUserId, isAuthenticated, isLoading: authLoading, logout } = useAuth();
  ...
  useEffect(() => {
    ...
    const fetchUserProfile = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(errorData.message || `Failed to fetch user profile for ID: ${profileUserId}`);
        }
    ...
    fetchUserProfile();
  }, [profileUserId, authLoading, isAuthenticated, router, currentUserId]);
  ...
  const handleUpload = async (e: FormEvent) => {
    ...
    try {
      ...
      if (!currentUserId) {
        setUploadError('User ID is not available. Please try logging in again.');
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-gray-900 text-gray-100">
        <div className="flex flex-col items-center space-y-6">
          ...
          <p className="text-xl font-semibold tracking-tight">Loading profile...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="flex flex-col min-h-screen bg-gray-900 text-gray-100 font-sans">
      <header className="flex items-center justify-between p-4 bg-gray-800 border-b border-gray-700 shadow-sm">
        <h1 className="text-2xl font-bold text-gray-100 tracking-tight">
          {isCurrentUserProfile ? "Your Profile" : `${userProfile.firstName} ${userProfile.lastName}'s Profile`}
        </h1>
        ...
        {!isCurrentUserProfile && (
          <Link href="/chat" className="px-4 py-2 bg-blue-600 text-gray-100 rounded-lg hover:bg-blue-700 transition-all duration-200 text-sm font-medium cursor-pointer">
            Back to Chat
          </Link>
        )}
      </header>
      ...
      <main className="flex-1 flex flex-col items-center justify-center p-6">
        <div className="w-full max-w-md bg-gray-800 rounded-xl shadow-lg p-8 space-y-6">
          ...
          {isCurrentUserProfile && (
            <form onSubmit={handleUpload} className="space-y-5 border-t border-gray-700 pt-6">
              ...
              {uploadMessage && (
                <p className="text-green-400 text-sm font-medium">{uploadMessage}</p>
              )}
              {uploadError && (
          ...
          <div className="mt-8 border-t border-gray-700 pt-6 flex flex-col sm:flex-row justify-center items-center space-y-4 sm:space-y-0 sm:space-x-6 text-sm">
            <Link href="/chat" className="text-blue-400 hover:text-blue-300 font-semibold transition-colors duration-200 cursor-pointer">
              Back to Public Rooms
            </Link>
            {isCurrentUserProfile && (
              <Link href="/chat/users" className="text-blue-400 hover:text-blue-300 font-semibold transition-colors duration-200 cursor-pointer">
                Find Users
              </Link>
            )}
            {!isCurrentUserProfile && (
              <Link href={`/profile/${currentUserId}`} className="text-blue-400 hover:text-blue-300 font-semibold transition-colors duration-200 cursor-pointer">
                Go to Your Profile
              </Link>
            )}
          </div>
